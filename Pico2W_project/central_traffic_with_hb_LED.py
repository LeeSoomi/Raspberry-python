...
# - 원본 로직/포맷 유지
# - 내 방향(MY_DIR)만 물리 LED 표시(GREEN/YELLOW/RED)
# - 콘솔에는 내 방향 기준 '남은 시간'을 역카운트로 한 줄만 출력(겹침 방지)
...
def mydir_phase_and_display_seconds(tl, t_now, my_dir, total=TOTAL):
    """
    반환: (phase_for_led, seconds_for_display)
      - GREEN 구간: ("GREEN", GREEN 남은 시간)
      - GREEN 마지막 1초: ("YELLOW", 다음 GREEN까지 대기 시간)
      - 그 외: ("RED", 다음 GREEN까지 대기 시간)
    """
...
try:
    while True:
        ...
        for t_now in range(TOTAL, 0, -1):
            ...
            # === 내 방향만 LED/숫자 갱신 (한 줄 덮어쓰기) ===
            phase, secs = mydir_phase_and_display_seconds(tl, t_now, MY_DIR, TOTAL)
            traffic_light.set_state(phase)
            msg = f"[{MY_DIR}] {phase:<6} 남은시간: {secs:2d}s"
